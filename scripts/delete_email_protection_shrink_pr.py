...
TRIAGE_PATH = Path(
    "/home/qcweb/defiliban/wave3_email_protection_delete_triage_2026-05-25.csv"
)
LOG_PATH = Path(
    "/home/qcweb/defiliban/wave3_email_protection_shrink_pr_delete_results_2026-05-25.csv"
)
TARGET_BUCKET = "delete_if_you_want_to_shrink_pr"
...
def load_targets() -> list[dict]:
    ...
    with TRIAGE_PATH.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            if row["action_bucket"] != TARGET_BUCKET:
                continue
...
def map_post(url_slug: str) -> dict:
    ...
    posts = response.json()
    if len(posts) != 1:
        return {"post_id": "", "title": "", "status": "lookup_failed"}
    post = posts[0]
    return {
        "post_id": post["id"],
...
def main() -> None:
    ...
    for target in targets:
        ...
        if not mapped["post_id"]:
            ...
            row["detail"] = "Could not map slug to a single published post."
            results.append(row)
            continue
        try:
            deleted = client.call(
